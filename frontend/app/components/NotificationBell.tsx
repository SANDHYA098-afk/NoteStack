  ...
  return (
    <div ref={ref} className="relative">
      ...
      >
        ...
        {unread > 0 && (
          <span
            className="absolute -top-2 -right-2 bg-[var(--pink)] text-white text-[10px] font-bold min-w-[18px] h-[18px] flex items-center justify-center px-1 border-[1.5px] border-[var(--border)]"
      ...
      {open && (
        <div
          className="absolute top-full right-0 mt-3 w-[calc(100vw-32px)] sm:w-[320px] max-w-[320px] bg-[var(--bg-card)] border-[2.5px] border-[var(--border)] overflow-hidden animate-scale-in z-50"
        ...
        >
          <div className="flex items-center justify-between px-4 py-3 border-b-[2px] border-[var(--border)] bg-[var(--yellow)]/20">
            ...
            {unread > 0 && (
              <button onClick={markAllRead} className="text-xs font-bold text-[var(--pink)] hover:underline" style={{ fontFamily: 'var(--font-body)' }}>
                Mark all read
              </button>
            )}
          </div>
            ...
            {notifications.length === 0 ? (
              ...
              notifications.map(n => (
                <div
                  ...
                  className={`px-4 py-3 border-b border-dashed border-[var(--border-light)] cursor-pointer transition-colors flex gap-3 items-start ${
                    n.read ? 'opacity-50' : 'bg-[var(--yellow)]/10 hover:bg-[var(--yellow)]/20'
                  }`}
                >
